...
#$browser->proxy(['http', 'ftp'], 'http://domain.com:8080/');
...
sub urlWeight {
	...
	#my $pattern = "google\.com\/aclk"; #use for Google
	...
	while(<TEMP2>) {
		...
		if ($_) {
			...
			if ($outputText =~ m/$pattern/) {
				$outputText =~ s/\<a href\=\"(\S+)\" h=(\S+)\"\>/$1/; #remove extra stuff around the url itself #unique to bing, leave it for now
